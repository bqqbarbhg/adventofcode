...
for index, node in enumerate(graph.values()):
	node.max_edge = max(edge[1] for edge in node.edges)
	node.mask = 1 << index
...
def astar(graph):
	...
	graph_max = sum(node.max_edge for node in graph.values())

	work = [(-graph_max, State(node, node.mask, 0, graph_max - node.max_edge, [node])) for node in graph.values()]
	heapq.heapify(work)
	...
	while work:
		...
		for edge in state.node.edges:
			...
			estimate = state.estimate - node.max_edge
