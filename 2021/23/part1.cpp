...
static const size_t step_cost[8] = { 1, 1, 10, 10, 100, 100, 1000, 1000 };
static const uint8_t room_x[8] = { 3, 3, 5, 5, 7, 7, 9, 9 };
static const uint8_t room_y = 2;
static const char names[] = "AABBCCDD";
...
{
	uint64_t a, b;
	bool operator==(const MapHash &rhs) const { return a == rhs.a && b == rhs.b; }
...
struct MapHasher
{
	std::size_t operator()(const MapHash &mh) const noexcept {
		size_t a = std::hash<uint64_t>()(mh.a);
		size_t b = std::hash<uint64_t>()(mh.b);
...
{
	Pos pods[8];
	uint8_t pods_to_move;
	uint8_t pods_to_return;
	size_t cost;
	char grid[5][14];
...
MapHash map_to_hash(const Map &map)
{
	...
	for (size_t index = 0; index < 8; index++) {
		Pos pos = map.pods[index];
		a |= (uint64_t)((pos.x&0xfu) | (pos.y&0xfu) << 4u) << (index * 8);
	}
	b = (uint64_t)map.pods_to_move | (uint64_t)map.pods_to_return << 8;
	return { a, b };
}
