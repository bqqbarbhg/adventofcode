(module
    ...
    (func $main (export "_start")
        ...
        (local.set $argpos (i32.const 10004))

        (local.set $mul_trees (i32.const 1))

        (block (loop

            ;; Break if we reach the end of the arguments
            (br_if 1 (i32.eqz (i32.load (local.get $argpos))))

            ;; Parse dx and dy
            (local.set $dx (call $parse_number (i32.load (i32.add (local.get $argpos) (i32.const 0)))))
            (local.set $dy (call $parse_number (i32.load (i32.add (local.get $argpos) (i32.const 4)))))
            (local.set $argpos (i32.add (local.get $argpos) (i32.const 8)))
            ...
            (local.set $mul_trees (i32.mul (local.get $mul_trees) (local.get $trees)))
            (br 0)
        ))

        ;; Measure the width
        (call $print_number (local.get $mul_trees))
    )
)
